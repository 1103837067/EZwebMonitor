/**
 * EZwebMonitor
 * - FPS/Memory overlay, draggable
 * - R/S sampling, 5s/10s/15s window
 * - CSV recording (adds jank,big_jank,delta_ms)
 * - Dynamic Y ±10% with smooth range animation
 * - Hover tips
 * - jank/big-jank detection:
 *    condA: delta > 2 * avg(prev 3 frames)
 *    AND (delta > 2 * movieFrame OR delta > 3 * movieFrame for big)
 *    where movieFrame = 1000/24 ≈ 41.67ms
 * - Jank marks on FPS chart: yellow (jank), red (big jank)
 */
(function installFPSMemoryOverlay(){
  if (window.__fpsOverlayInstalled) { try{ window.__perfOverlay?.remove?.(); }catch(e){} }
  ...
  closeBtn.onclick=()=>{ cancelAnimationFrame(rafId); clearInterval(timerId); box.remove(); delete window.__perfOverlay; window.__fpsOverlayInstalled=false; };
  ...
  function drawPanel(row, points, label, color, range, janks, showJankCounts){
    ...
    if (janks && janks.length){
      const sizeJ = 7;   // jank box (yellow)
      const sizeB = 11;  // big jank box (red)
      janks.forEach(ev=>{
